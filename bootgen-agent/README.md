...
- **易于扩展**：后续加入"人工确认"节点、"错误重试"节点只需加节点和边，不影响其他逻辑
...
LangChain Agent 是黑盒——你只能看到最终输出，看不到中间的决策过程。LangGraph 把每一步显式建模为节点，状态流转清晰可见，更适合生产环境和后续迭代。两个版本都保留在仓库里，可以直接对比代码量和结构差异。
